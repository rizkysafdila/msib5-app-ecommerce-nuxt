...
<template>
  <div class="flex flex-wrap md:flex-nowrap justify-between gap-6">
      ...
      <div
        ...
        <div class="flex flex-col justify-between w-full md:w-3/5 h-fit md:h-full p-3">
          ...
          <div class="flex justify-between items-center mt-10 md:mt-0">
            ...
            <button @click="cartStore.REMOVE_PRODUCT(item.id)"
    ...
    <div v-if="cartStore.getSubTotalPrice !== 0" class="w-full flex md:w-5/12">
      <div class="w-full h-fit flex flex-wrap bg-white border border-gray-200 rounded-xl p-5">
        <div class="w-full flex justify-between mb-2 lg:mb-1">
          <div class="text-gray-900 font-normal">Sub Total</div>
          <div class="text-gray-900 text-lg md:text-xl text-right font-semibold">
            {{ usdCurrency(cartStore.getSubTotalPrice) }}
          </div>
        </div>
        <div class="text-gray-400 text-sm md:text-base font-normal md:w-[190px] mb-5">
          Shipping and taxes calculated at checkout
        </div>
        <button @click="router.push('/checkout/success')"
