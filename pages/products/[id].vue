  ...
  <section>
    <div class="container mx-auto lg:gap-8 xl:gap-0 py-12 lg:pt-16 lg:pb-[72px] px-8">
      <div class="flex justify-between items-center">
        ...
        <h1 class="text-gray-900 text-center text-2xl md:text-3xl lg:text-[40px] font-semibold">
          Product Detail
        </h1>
    ...
    <div class="container mx-auto lg:gap-8 xl:gap-0 py-12 lg:pt-16 lg:pb-[72px] px-8">
      <div class="w-full flex flex-wrap lg:flex-nowrap gap-5">
        <div class="flex flex-col gap-4 w-full lg:w-6/12">
          <div class="bg-gray-100 w-full p-5 lg:max-h-[546px] flex justify-center items-center rounded-xl">
            <NuxtImg class="rounded-xl h-full object-scale-down" :src="product.image" alt="bottle" />
          </div>
          <div class="flex gap-4">
            <div
              ...
              v-for="i in 4">
              <NuxtImg class="h-full object-scale-down rounded-md lg:rounded-xl" :src="product.image" alt="bottle" />
            </div>
          </div>
        </div>
        <div class="w-full lg:w-6/12 flex flex-col gap-8">
          ...
          <div>
            ...
            <ul class="flex flex-wrap md:flex-nowrap w-full gap-6">
              <li v-for="color in ['red', 'green', 'blue']">
                <input type="radio" :id="`color-${color}`" class="hidden peer" required />
                <label :for="`color-${color}`"
                  ...
                  <div class="block">
                    <div class="w-full text-gray-600 text-lg font-semibold capitalize">
                      {{ color }}
                    </div>
                  </div>
                </label>
          ...
          <div>
            ...
            <ul class="flex w-full gap-4">
              <li v-for="size in ['S', 'M', 'L', 'XL']" :key="size">
                <input type="radio" :id="`size-${size}`" class="hidden peer" required />
                <label :for="`size-${size}`"
          ...
          <div>
            ...
            <div class="flex gap-4">
              <ProductCounterButton icon="tabler:minus" @count="decrement" :disabled="quantity <= 1" />
